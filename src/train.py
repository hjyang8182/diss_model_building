...
class_weights = utils.return_class_weights()
train_files, valid_files, test_files = utils.split_train_valid_test()
...
spo2_delta_data = utils.prepare_data(feature = 'spo2', p_train = 0.7, p_valid = 0.15, use_delta = True)
audio_data_full = utils.prepare_data(feature = 'audio')

spo2_raw_data_subset = utils.prepare_data_subset(feature = 'spo2',train_length= 24000, valid_length = 1200, test_length = 1200, train_batch_size= 128, valid_test_batch_size= 32)
delta_model_data = utils.prepare_data_subset(feature = 'spo2', train_length= 24000, valid_length = 1200, test_length = 1200, train_batch_size= 128, valid_test_batch_size= 32, use_delta = True)
...
mel_spec_data = utils.load_mel_spec_images(train_length= 8000, valid_length = 800, test_length = 800, train_batch_size= 128, valid_test_batch_size= 32)
...
def test_model(model_fn, model_data): 
    ...
    # Fit the model 
    history = model.fit(train_data, steps_per_epoch = TRAIN_STEPS_PER_EPOCH, epochs = 50, verbose = 1, validation_data = valid_data, callbacks = [early_stopping], validation_steps = VALID_STEPS_PER_EPOCH)
...
def train_model(model_fn, model_name, model_data): 
    ...
    # Fit the model 
    history = model.fit(train_data, steps_per_epoch = TRAIN_STEPS_PER_EPOCH, epochs = 50, verbose = 1, validation_data = valid_data, callbacks = [early_stopping, checkpoint], validation_steps = VALID_STEPS_PER_EPOCH)
...
def train_model_full_data(model_fn, model_name, model_data): 
    ...
    # Fit the model 
    history = model.fit(train_data, steps_per_epoch = FULL_TRAIN_STEPS_PER_EPOCH, epochs = 50, verbose = 1, validation_data = valid_data, callbacks = [early_stopping, checkpoint], validation_steps = FULL_VALID_STEPS_PER_EPOCH)
...
def load_and_train(model_fn, model_name, model_data, initial_epoch, custom_objects = {}): 
    ...
    # Fit the model 
    history = model.fit(train_data, initial_epoch = initial_epoch, steps_per_epoch = FULL_TRAIN_STEPS_PER_EPOCH, epochs = 50, verbose = 1, validation_data = valid_data, callbacks = [early_stopping, checkpoint], validation_steps = FULL_VALID_STEPS_PER_EPOCH)
