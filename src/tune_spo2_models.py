...
from tensorflow.keras.layers import GlobalAveragePooling1D, MaxPool1D, InputLayer, Reshape, LSTM, Bidirectional, Dense, Conv1D, Flatten, GRU, BatchNormalization
...
def bilstm_windowed_delta_model(hp): 
    ...
    for i in range(num_dense_layers):
        model.add(Dense(base_dense_units, activation = 'relu'))
...
spo2_windowed_data = data_loader.load_subset_data('spo2', window_spo2 = True)
spo2_data = data_loader.load_subset_data('spo2')


tune(lstm_delta_model, spo2_windowed_data, 'spo2_lstm_dense_units', data_loader.TRAIN_STEPS_PER_EPOCH, data_loader.VALID_STEPS_PER_EPOCH)
# tune(bilstm_windowed_delta_model, spo2_windowed_data, 'spo2_bilstm_dense_units', data_loader.TRAIN_STEPS_PER_EPOCH, data_loader.VALID_STEPS_PER_EPOCH)

# visualize_hp(cnn_1d_model, 'spo2_cnn_tune_dense')
